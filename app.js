...
fetch('./data/data.json')
  ...
  .then((arrayOfStoryOjects) => {
    ...
    let storyTitles = [];
    for (let i in arrayOfStoryOjects) {
    ...
    storyButtons.addEventListener('click', (e) => {
      ...
      form.addEventListener('submit', (e) => {
        ...
        let wordsAsString = words.join();

        // send words to the proxy server using axios
        axios.post('http://localhost:3000/filter-words', {
          words: wordsAsString,
        })
        .then((response) => {
          let result = response.data;
          words = result.censored_content.split(',');

          // create story html
          let html =
            '<div class="result"><img src="public/3.png" width="100" height="100"/> <div class="story-final">';
          for (let i in storyPieces) {
            html += storyPieces[i];
            if (i < words.length) {
